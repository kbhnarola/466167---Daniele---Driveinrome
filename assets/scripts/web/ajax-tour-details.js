        ...
        jQuery.ajax({
            ...
            success: function (r) {
                ...
                if (r.success) {
                    ...
                    console.log('get_total_price_b'+get_total_price_b)
                    var extra_price = jQuery("#tour_extra_cost").val();
                    console.log('extra_price'+extra_price)
                    var final_total_price = get_total_price_b + parseFloat(extra_price);

                    jQuery(".single_tour_price").removeClass('selectedThumb');
                    jQuery(".single_tour_price").html('€ ' + (final_total_price).toFixed(2));
                    ...
                    jQuery.each(r.data, function (j, item) {
                        ...
                        $('#extra_price').val(item.extra_cost);
                                ...
                                if (item.tour_availability == 0) {
                                    ...
                                    if (atob(jQuery("#tour_type_id").val()) == 3) {
                                        ...
                                        if (infants) {
                                            ...
                                            if (item.variation_id == variation_id) {
                                                var final_price  = (parseFloat(item.price) + parseFloat(item.extra_cost));
                                                item_price = '€ ' + (final_price).toFixed(2);
                                                jQuery('div[ data-tour-date="' + item.tour_date + '"]').html(item_price);
                                                date_array.push(item.tour_date);
                                            }
                                        }
                                    ...
                                    if (atob(jQuery("#tour_type_id").val()) == 8) {
                                        ...
                                        var final_extra_price  = 0;
                                        if(item.extra_cost){
                                            final_extra_price  = parseFloat(item.extra_cost);
                                        }
                                        var ttl_person_package_v = parseInt(total_person) + parseInt(inf_member);
                                        if (ttl_person_package_v == 1 && item.variation_id == 14) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 2 && item.variation_id == 15) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 3 && item.variation_id == 16) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 4 && item.variation_id == 17) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 5 && item.variation_id == 18) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 6 && item.variation_id == 19) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        if (ttl_person_package_v == 7 && item.variation_id == 20) {
                                            total_price_v = item.price;
                                            pkg.push(item.price);
                                        }
                                        ...
                                        if (item.variation_id == 21) {
                                            if (ttl_person_package_v == 8) {
                                                ...
                                                console.log(pkg);
                                                var final_price  = ((parseFloat(pkg[0]) + final_extra_price)).toFixed(2);
                    ...
                    if(extra_price != '' && extra_price > 0 ){
                        final_extra = extra_price;
                    }
                    var final_price = get_total_price + parseFloat(final_extra);

                    //jQuery("#total_price").html(' € '+ (Math.round(get_total_price)));
                    jQuery(".total_price_single_tour").html(' € ' + (final_price.toFixed(2)));
...
jQuery(document).on('click', ".popup-btn", function () {
    jQuery('.slider-video').show();
    jQuery('.gallery_tour_slider').hide();
});
jQuery(document).on('click', ".slider-video button.close", function () {
    jQuery('.gallery_tour_slider').show();
    jQuery('.slider-video').hide();
    jQuery('.slider-video iframe').attr('src', jQuery('.slider-video iframe').attr('src'));
});
