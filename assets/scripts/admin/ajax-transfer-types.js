...
jQuery("#addTransferTypeform").validate({
    errorElement: 'span',
    rules:{
        transfer_type: {
            required:true,
            ...
            noHTML:true,
            remote: {
            			url: BASE_URL+"transfer_types/istypeExists",
                        type: "POST",
                        data: {
                            record_id: function() {
                                return $('#transfer_type_id').val();
                            },
                        }
                     },
            maxlength:20
    ...
    submitHandler:function(form){
        ...
        if(jQuery('#transfer_type_id').val()) {
            var ajaxSubmit=BASE_URL+"transfer_types/edit";
        } else {
            var ajaxSubmit=BASE_URL+"transfer_types/add"
        }
            jQuery.ajax({
                url: ajaxSubmit,
                type: 'POST',
                data:jQuery("#addTransferTypeform").serialize(),
                dataType:'JSON',
                beforeSend: function () {
                      jQuery("#addTransferTypeform").hide();
                      jQuery("#loader_cont").removeClass('hidden');    
                }, 
                success: function(response) {

                    jQuery("#addTransferTypeform").show();
                    jQuery("#loader_cont").addClass('hidden');  
                        ...
                        if(response.success == false) {
                          ...
                          jQuery("#addTransferTypeform")[0].reset();
                          $('#add_transfer_type_modal').modal('hide');

                          $('#transfer_type_list_table').DataTable().ajax.reload();  
...
$(document).ready(function(){
    ...
    $('#transfer_type_list_table').DataTable({
        ...
        "ajax": {
            "url": BASE_URL+'transfer_types/getLists/',
            "type": "POST",
            // columnsDef: [
            //             'RecordID', 'transfer_type', 'status', 'action'],
        },
        "columns": [
...
function change_status(obj)
{
    ...
    $.ajax({
        url:BASE_URL+'transfer_types/update_status',
        type: 'POST',
        data: {
            transfer_type_id: obj.id,
            is_active:checked
        },
        dataType:'JSON',
        success: function(response) 
...
{ 
    ...
    function()
    {
        ...
        $.ajax({
            url:BASE_URL+'transfer_types/delete',
            type: 'POST',
            data: {
                transfer_type_id:obj.id
            },
...
{   
    jQuery(".load-main").removeClass('hidden');
    $.ajax({
        url:BASE_URL+'transfer_types/get_record_byID',
        type: 'POST',
        data: {
            transfer_type_id: obj.id,
        },
        ...
        success: function(data) 
        {
            ...
            if(data.id){
                ...
                jQuery('#transfer_type_id').val(btoa(data.id));
                
                jQuery("#transfer_type").val(data.title);
                
                jQuery('.addModalTitle').addClass('hidden');
                jQuery('.editModalTitle').removeClass('hidden');

                jQuery("#add_transfer_type_modal").modal('show');
            } else {
