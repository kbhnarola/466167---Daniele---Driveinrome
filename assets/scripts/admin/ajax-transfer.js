jQuery(document).ready(function(){
    ...
    if(jQuery("#transfer_list_table").length){
        ...
        $('#transfer_list_table').DataTable({
            ...
            "ajax": {
                "url": BASE_URL+'transfers/getLists/',
                "type": "POST"
            },
            "columns": [
                    { data: 'RecordID', 'sortable': false,"orderable": false }, //0
                    { data: 'transfer_name'} , //1
                    { data: 'transfer_type'} , //2
                    { data: 'transfer_category'} , //3                   
                    { data: 'unique_code'} , //4
                    { data: 'duration'} , //5
            ...
            "columnDefs": [ 
                ...
                'title': 'Actions',
                 ...
                 'render': function (data, type, full, meta){
                    //var id = data.id;
                    return '<a href="'+BASE_URL+'transfers/edit/'+data+'" data-popup="tooltip" data-placement="top"  title="edit" id="edit'+data+'" class="text-info"><i class="icon-pencil7"></i></a>&nbsp;&nbsp;<a data-popup="tooltip" data-placement="top"  title="delete" href="javascript:" onclick="delete_record(this)" class="text-danger delete" id="'+data+'"><i class=" icon-trash"></i></a>';
                 }
                },
                {
                'targets': [7],
...
function change_status(obj)
{
    ...
    jQuery.ajax({
        url:BASE_URL+'transfers/update_status',
        type: 'POST',
        data: {
            transfer_id: obj.id,
            is_active:checked
    ...
    function()
    {
        ...
        $.ajax({
            ...
            success: function(response)
            {
                ...
                if(response.success)
                {                        
                    $('#transfer_list_table').DataTable().ajax.reload();     
