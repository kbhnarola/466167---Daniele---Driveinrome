...
jQuery("#addTransferServiceform").validate({
        rules:{
            ...
            transfer_name:{
                ...
                noHTML:true,
                remote: {
                        url: BASE_URL+"admin/transfers/isTransferServiceExists",
                        type: "POST",
                        data: {
                            record_id: function() {
            ...
            transfer_unique_code:{
                ...
                noHTML:true,
                remote: {
                            url: BASE_URL+"admin/transfers/isTransferCodeExists",
                            type: "POST",
                            data: {
                                record_id: function() {
                                    return $('#transfer_id').val();
...
jQuery("#transfer_type").select2().change(function() {
    ...
    jQuery.ajax({
        url:BASE_URL+'admin/transfers/getTransferVariantion',
        type: 'POST',
        data: {
...
jQuery(document).on("click","#reset_transfer",function(){
        
    jQuery('#transfer_category').val('').trigger('change');
    
    jQuery('#transfer_type').val('').trigger('change');

    var validator = $("#addTransferServiceform").validate();
    validator.resetForm();
    jQuery("#addTransferServiceform")[0].reset();
