jQuery('.addextraservices').click(function () {
    ...
    jQuery(".b_price").show();
    var validator = $("#addExtraCostform").validate();
    validator.resetForm();
    jQuery('.addModalTitle').removeClass('hidden');
    jQuery('.editModalTitle').addClass('hidden');
    jQuery('#extra_cost_id').val('');
    jQuery("#addExtraCostform")[0].reset();
});
...
jQuery("#addExtraCostform").validate({
    errorElement: 'span',
    rules: {
        extra_cost_title: {
            required: true,
            ...
            noHTML: true,
            remote: {
                url: BASE_URL + "tour_extra_cost/istypeExists",
                type: "POST",
                data: {
                    record_id: function () {
                        return $('#extra_cost_id').val();
                    },
                }
            },
            maxlength: 500
    ...
    messages: {
        extra_cost_title: {
            required: "Please Enter Tour Extra Cost Title",
            remote: "Tour Extra Cost Title already exist"
        }
    },
    submitHandler: function (form) {

        if (jQuery('#extra_cost_id').val()) {
            var ajaxSubmit = BASE_URL + "tour_extra_cost/edit";
        } else {
            var ajaxSubmit = BASE_URL + "tour_extra_cost/add"
        }
        jQuery.ajax({
            url: ajaxSubmit,
            type: 'POST',
            data: jQuery("#addExtraCostform").serialize(),
            dataType: 'JSON',
            beforeSend: function () {
                jQuery("#addExtraCostform").hide();
                jQuery("#loader_cont").removeClass('hidden');
            },
            success: function (response) {

                jQuery("#addExtraCostform").show();
                ...
                if (response.success == false) {
                    ...
                    jQuery("#addExtraCostform")[0].reset();
                    $('#add_extra_cost_modal').modal('hide');

                    $('#extra_cost_list_table').DataTable().ajax.reload();
...
$(document).ready(function () {
    ...
    $('#extra_cost_list_table').DataTable({
        ...
        "ajax": {
            "url": BASE_URL + 'tour_extra_cost/getLists/',
            "type": "POST",
            // columnsDef: [
...
function change_status(obj) {
    ...
    $.ajax({
        url: BASE_URL + 'tour_extra_cost/update_status',
        type: 'POST',
        data: {
            extra_cost_id: obj.id,
            is_active: checked
        },
        dataType: 'JSON',
        success: function (response) {
...
function delete_record(obj) {
    ...
    swal({
        ...
        function () {
            ...
            $.ajax({
                url: BASE_URL + 'tour_extra_cost/delete',
                type: 'POST',
                data: {
                    extra_cost_id: obj.id
                },
...
function edit_record(obj) {
    jQuery(".load-main").removeClass('hidden');
    $.ajax({
        url: BASE_URL + 'tour_extra_cost/get_record_byID',
        type: 'POST',
        data: {
            extra_cost_id: obj.id,
        },
        ...
        success: function (data) {
            ...
            if (data.id) {
                ...
                jQuery("#addExtraCostform")[0].reset();
                jQuery('#extra_cost_id').val(btoa(data.id));

                jQuery("#extra_cost_title").val(data.title);
                ...
                jQuery("#add_extra_cost_modal").modal('show');
            } else {
