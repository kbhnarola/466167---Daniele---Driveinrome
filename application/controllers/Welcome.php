...
class Welcome extends CI_Controller
{
	...
	public function send_quick_quote_details()
	{
		if($this->input->post('captcha'))
		{
			...
			$url 					= 'https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($secret_key) .  '&response=' . urlencode($this->input->post('captcha'));
			...
			if($response_keys["success"]) {
				...
				$placeholder = ['{name}', '{email}', '{phone}', '{find_us}', '{notes}', '{welcome_image}', '{date}', '{number_of_passengers}', '{interest_service}', '{name_of_ship}'];
				$form_data   = [ucwords(trim($_POST['qfullname'])), trim($_POST['qemail']), trim($_POST['qphoneNumber']), trim($_POST['howDidFind']), trim($_POST['notes']), EMAIL_WELCOME_PNG,trim($_POST['qdate']),trim($_POST['number_of_passengers']),trim($_POST['interest_service']),trim($_POST['name_of_ship'])];
				...
				$quick_quote_active_campaign = DEFAULT_ACTIVE_CAMPAIGN_ID;
	...
	public function send_contact_us_details()
	{
		if($this->input->post('captcha'))
		{
			...
			if($response_keys["success"]) {
				...
				$placeholder = ['{name}', '{email}', '{phone}', '{how_did_you_find_us}',  '{number_of_passenger}', '{message}', '{welcome_image}', '{origin}', '{date}', '{interest_service}', '{name_of_ship}'];
				$form_data   = [ucwords(trim($_POST['fullname'])), trim($_POST['email']), trim($_POST['phoneNumber']), trim($_POST['how_did_you_find_us']), trim($_POST['noOfPassenger']), trim($_POST['message']), EMAIL_WELCOME_PNG, '<a href="' . $_POST['origin'] . '">' . $_POST['origin'] . '</a>',trim($_POST['c_dates']),trim($_POST['interest_service']),trim($_POST['name_of_ship'])];
				...
				$contact_form_active_campaign = DEFAULT_ACTIVE_CAMPAIGN_ID;
				...
				$email_subject = 'Contact form | '. ucwords(trim($_POST['fullname']));
				if(!empty($_POST['interest_service'])){
					$email_subject .= ', '.trim($_POST['interest_service']);
				}
				$data['subject'] = $email_subject;
	...
	public function send_email_to_admin_for_subscribe_newsletter()
	{
		if($this->input->post('g-recaptcha-response'))
		{
			...
			if($response_keys["success"]) {
				...
				$placeholder = $form_data = array();
				$email_subject = $admin_email_subject = 'Driver In Rome';
				$get_email_template = get_email_template('subscribe-newsletter');

				$get_email_body = (!empty($get_email_template['message'])) ? $get_email_template['message'] : '';

				if (!empty($get_email_template['subject'])) {
					$admin_email_subject = $get_email_template['subject'];
				}
				...
				$get_new_admin_email_body = str_replace($placeholder, $form_data, $get_email_body);
				...
				if ($send_subscribe_email) {
					...
					// send email to admin
					$data['email_body'] = $get_new_admin_email_body;
					$data['subject'] = $admin_email_subject;
	...
	{
		$data['tour_details'] = $this->welcome->get_tour_list_by_city_slug(($city_slug), array('3'), $small_group_tours_in_city_tours = true);
	...
	{
		$data['tour_details'] = $this->welcome->get_transfer_tour_list_by_city_slug(($city_slug), array('9', '10'));
	...
	public function review(){
		$data['review_details'] = $this->welcome->get_cms_page_details_slug('review');
		$data['title'] = $data['review_details']['meta_title'];
		$data['meta_description'] = $data['review_details']['meta_description'];
		$data['meta_keyword'] = $data['review_details']['meta_keyword'];

		$data['review_platform_details'] = unserialize(get_settings('platform_review_detail'));
		
		$this->load->frontpage('frontpages', 'review', $data);
	}
}
?>