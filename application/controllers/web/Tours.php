...
class Tours extends Frontend_Controller
{
	...
	public function index()
	{
		...
		if (is_array($toursData) && sizeof($toursData) > 0) {
				...
				$extra_costs_price = 0;				
				if ($toursData['extra_costs_id']) {
					$extra_cost_array = get_extra_cost($toursData['extra_costs_id']);
					if (is_array($extra_cost_array) && sizeof($extra_cost_array) > 0) {
						foreach ($extra_cost_array as $ex) {
							$extra_costs_price = $extra_costs_price + $ex['price'];
						}
					}
				}				
				$toursData['extra_cost'] = $extra_costs_price;			
	...
	public function transfer_tours($city_slug)
	{
		...
		$data['video_status'] = 0;
		if($meta_details != ''){
			$youtube_embed_video_url = $meta_details[0]['youtube_embed_video_url'];
			if($youtube_embed_video_url != ''){
				$data['video_url'] = $youtube_embed_video_url;
				$data['video_status'] = 1;
			}
		}
		$data['video_modal'] = $this->load->view('frontpages/city_video_modal', $data, TRUE);
	...
	public function get_tour_price()
	{
		...
		if (is_array($result) && sizeof($result) > 0) {
			foreach ($result as $rkey => $res) {
				$extra_costs_price = 0;
				$toursData = $this->tours->get_tour_details_byId($res['tour_id']);
				if ($toursData['extra_costs_id']) {
					$extra_costs_price = 0;
					$extra_cost_array = get_extra_cost($toursData['extra_costs_id']);
					if (is_array($extra_cost_array) && sizeof($extra_cost_array) > 0) {
				...
				$result[$rkey]['extra_cost'] = $extra_costs_price;
			}
	...
	{
		if($this->input->post('rqquote_hiddenRecaptcha'))
		{
			$secret_key 			= get_settings('gr_secret_key');

			$url 					= 'https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($secret_key) .  '&response=' . urlencode($this->input->post('rqquote_hiddenRecaptcha'));

			$grresponse 				= file_get_contents($url);
			$response_keys 			= json_decode($grresponse,true);
			if($response_keys["success"]) {
				$username = ucwords(trim($this->input->post('user_name')));
				$user_email = $this->input->post('user_email');
				$phoneNumber = $this->input->post('phoneNumber');
				$contactHowDidFind = $this->input->post('contactHowDidFind');
				...
				$this->form_validation->set_rules('contactHowDidFind', 'How Did Find', 'trim|required');
				if ($this->form_validation->run() == FALSE) {
					...
					if ($toursData['tour_availability'] == 0) {
						...
						$placeholder = ['{username}', '{tour_date}', '{passenger}', '{tour_email_description}', '{oops_image}', '{contactHowDidFind}', '{phoneNumber}'];
						$form_data   = [$username, date('d M Y', strtotime($toursData['request_date'])), $toursData['total_passenger'], $tour_email_description, EMAIL_OOPS_PNG, $contactHowDidFind, $phoneNumber];
						$email_template_name = 'get-quote-oops';
						$tour_availability = "Not Available";
						$tour_rate = "-";
					} else {
						$get_email_template = get_email_template('get-quote');
						$placeholder = ['{username}', '{tour_date}', '{passenger}', '{tour_name}', '{rate_for_tour}', '{tour_email_description}', '{welcome_image}', '{contactHowDidFind}', '{phoneNumber}'];
						$form_data   = [$username, date('d M Y', strtotime($toursData['request_date'])), $toursData['total_passenger'], $toursData['toursData']['title'], '€ ' . $toursData['total_rate'], $tour_email_description, EMAIL_WELCOME_PNG, $contactHowDidFind, $phoneNumber];
						$email_template_name = 'get-quote';
						$tour_rate = '€ ' . $toursData['total_rate'];
					}
					$get_email_template_admin = get_email_template('get-quote-admin');
					$placeholder_admin = ['{username}', '{email}', '{enquiry_date}', '{passenger}', '{tour_name}', '{tour_code}', '{tour_rate}', '{tour_notes}', '{tour_availability}', '{welcome_image}', '{contactHowDidFind}', '{phoneNumber}'];
					$form_data_admin   = [$username, $user_email, date('d M Y', strtotime($toursData['request_date'])), $toursData['total_passenger'], $toursData['toursData']['title'],$toursData['toursData']['unique_code'], $tour_rate, $toursData['tour_notes'], $tour_availability, EMAIL_WELCOME_PNG, $contactHowDidFind, $phoneNumber];
	...
	public function add_ticket()
	{
		...
		if (is_array($toursData) && sizeof($toursData) > 0) {
			...
			$data['tour_availability'] = $tour_availability;
			$data['toursData']['total_tour_cost'] = $total_rate;
			// set the extra cost details into cart session
			$toursData = $this->tours->get_tour_details_byId($toursData['tour_id']);
			if ($toursData['extra_costs_id']) {
				$extra_cost_array = get_extra_cost($toursData['extra_costs_id']);
				if (is_array($extra_cost_array) && sizeof($extra_cost_array) > 0) {
					foreach ($extra_cost_array as $ex) {
						array_push($extra_cost_cart_array, array('title' => $ex['title'], 'price' => $ex['price']));
						$extra_costs_price += $ex['price'];
					}
				}
			}
			$data['toursData']['total_extra_cost'] = $extra_costs_price;
			$data['tour_extra_cost'] = $extra_cost_cart_array;
			...
			if ($ex_s > 0) {
				...
				$data = array(
					...
					'qty'     => 1,
					'price'   => $tour_session_list['total_rate'] + $extra_costs_price,
				...
				if (count($this->cart->contents()) > 0) {
					foreach ($this->cart->contents() as $c) {
						if (array_key_exists('tours_detail_data', $c)) {
							if ($c['id'] == $data['id']) {
								$data1 = $this->cart->update(array(
									'rowid' => $c['rowid'],
									'price'   => $tour_session_list['total_rate'] + $this->input->post('total_tour_upgrades_price') + $extra_costs_price,
									'name'    => $tour_session_list['toursData']['title'],
									'tours_detail_data' => $tour_session_list
								));
		...
		if ($this->input->post()) {
			if (array_key_exists('tour_upgrades', $this->input->post())) {
				...
				if (array_key_exists("colo_first_name", $this->input->post())) {
					...
					$ses_list['tour_upgrades_passenger_detail']['colo_birth_date'] =  $this->input->post('colo_birth_date[]');
					$ses_list['tour_upgrades_passenger_detail']['colo_birth_place'] =  $this->input->post('colo_birth_place[]');
				...
				$this->session->set_userdata('tours_data', $ses_list);
				$tour_session_list = $this->session->userdata('tours_data');
				$tour_total_rate = (float)$tour_session_list['total_rate'];
				// set the extra cost details into cart session
				$toursData = $this->tours->get_tour_details_byId($tour_session_list['toursData']['tour_id']);
				$extra_costs_price = 0;
				if ($toursData['extra_costs_id']) {
					$extra_cost_array = get_extra_cost($toursData['extra_costs_id']);
					if (is_array($extra_cost_array) && sizeof($extra_cost_array) > 0) {
				...
				$tour_session_list['toursData']['total_tour_cost'] = $tour_total_rate + $this->input->post('total_tour_upgrades_price');
				$tour_session_list['toursData']['total_extra_cost'] = $extra_costs_price;
				$tour_session_list['tour_extra_cost'] = $extra_cost_cart_array;
				// $tour_session_list = $this->session->userdata('tours_data');
				$this->session->set_userdata('tours_data', $ses_list);
				$data = array(
					'id'      => $tour_session_list['toursData']['tour_id'],
					'qty'     => 1,
					'price'   => $tour_total_rate + $this->input->post('total_tour_upgrades_price') + $extra_costs_price,
					'name'    => $tour_session_list['toursData']['title'],
					'tours_detail_data' => $tour_session_list
				);
				$is_tourId = 0;
				...
				// pr($tour_session_list, 1);
				$tour_session_list['toursData']['total_tour_cost'] =  $tour_total_rate + (float)$this->input->post('total_tour_upgrades_price');
				$tour_session_list['toursData']['total_extra_cost'] = $extra_costs_price;
				$tour_session_list['tour_extra_cost'] = $extra_cost_cart_array;
				$this->session->set_userdata('tours_data', $ses_list);
				if($ses_list['toursData']['total_tour_cost'] > 0 ){
					$sec_price = $ses_list['toursData']['total_tour_cost'] + $ses_list['toursData']['total_extra_cost'];
					$tour_total_rate  = $sec_price;
				}else{
					$tour_total_rate  = $tour_total_rate;
				}
				$data = array(
					'id'      => $tour_session_list['toursData']['tour_id'],
					'qty'     => 1,
					'price'   => $tour_total_rate,
					'name'    => $tour_session_list['toursData']['title'],
					'tours_detail_data' => $tour_session_list
				);
				$is_tourId = 0;
					...
					foreach ($this->cart->contents() as $c) {
						if (array_key_exists('tours_detail_data', $c)) {
							if ($c['id'] == $data['id']) {
								$data = $this->cart->update(array(
									'rowid' => $c['rowid'],
									'price'   => $tour_total_rate + (float)$this->input->post('total_tour_upgrades_price'),
									'name'    => $tour_session_list['toursData']['title'],
									'tours_detail_data' => $tour_session_list
								));
								$this->cart->product_name_rules = '[:print:]';
