...
class Transfer_tours extends Admin_Controller
{
    /**
    ...
    {
        parent::__construct();
        $this->load->model('Transfer_tours_model', 'transfer_tours');
    ...
    {
        $this->set_page_title(_l('manage_transfer_tours'));

        $data['content'] = $this->load->view('admin/transfer_tours/index', '', TRUE);
    ...
    public function getLists()
    {
        ...
        // Fetch tours's list
        $memData = $this->transfer_tours->getRows($_POST, $type = "transfer");
        ...
        foreach ($memData as $tours) {
            ...
            if ($tours->tour_type_id == 8) {
                if ($tours->duration > 1) {
                    $data[$j]['duration'] = $tours->duration . " Days";
                } else {
                    $data[$j]['duration'] = $tours->duration . " Day";
                }
            } else {
                if ($tours->duration > 1) {
                    $data[$j]['duration'] = $tours->duration . " Hours";
                } else {
                    $data[$j]['duration'] = $tours->duration . " Hour";
                }
            }
            if ($tours->rating == 0) {
                $data[$j]['ratings'] = "N/A";
            ...
            if ($tours->extra_costs_id) {
                $ex_costs = explode(",", $tours->extra_costs_id);
                $extra_costs_title = array();
                $extra_costs_array = get_extra_cost($tours->extra_costs_id);
                foreach ($extra_costs_array as $ex) {
                    // if (array_key_exists($ex, $extra_costs_array)) {
                    $extra_costs_title[] = $ex['title'];
                    // }
                }
                if (is_array($extra_costs_title) && sizeof($extra_costs_title) > 0) {
                    $data[$j]['extra_costs'] = implode(", ", $extra_costs_title);
                } else {
                    $data[$j]['extra_costs'] = "N/A";
                }
            } else {
                $data[$j]['extra_costs'] = "N/A";
            }
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->transfer_tours->countFiltered($_POST, $type = "transfer"),
            "recordsFiltered" => $this->transfer_tours->countFiltered($_POST, $type = "transfer"),
            "aaData" => $data,
        );
    ...
    public function add()
    {
        if ($this->input->post()) {
            ...
            // $this->form_validation->set_rules('video_url', 'Tour Video URL', 'trim|required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(admin_url('transfer-tours'));
            } else {
                ...
                foreach ($_FILES as $key => $value) {
                    ...
                    if ($key == "tour_banner_image") {
                        if ($value["name"]) {
                            ...
                            if ($this->upload->do_upload($key)) {
                                $data = array('upload_data' => $this->upload->data());
                                $tour_banner_image = $data['upload_data']['file_name'];
                            } else {
                                //$data['optional_document_upload_error'] = $this->upload->display_errors();
                                set_alert('error', current($this->upload->display_errors()));
                                redirect(admin_url('transfer-tours'));
                ...
                $tour_email_description = $this->input->post("tour_email_description");
                $duration = trim($this->input->post("duration"));
                $video_url = trim($this->input->post("video_url"));
                $active_campaign_automation_id = trim($this->input->post("active_campaign_automation"));
                $top_selling_tour = $this->input->post("top_selling_tour");
                $upselling_tour = $this->input->post("upselling_tour");
                $ratings = trim($this->input->post("rating"));
                $extra_services = $this->input->post("extra_services[]");
                $extra_costs = $this->input->post("extra_cost[]");
                $meta_title = $this->input->post("meta_title");
                $meta_keywords = $this->input->post("meta_keywords");
                $meta_description = $this->input->post("meta_description");

                // store is_city_tour data only small group tour type selected
                $is_city_tour = 0;
                if ($tour_type == 7) {
                    $is_city_tour = $this->input->post("is_city_tour");
                }
                ...
                if (array_key_exists("tour_faqs_question", $this->input->post())) {
                    ...
                    $tour_faqs_v = json_encode($tour_faqs, JSON_FORCE_OBJECT);
                } else {
                    $tour_faqs_v = "";
                }
                ...
                if (is_array($extra_costs) && sizeof($extra_costs) > 0) {
                    $extra_cost_id = implode(",", $extra_costs);
                } else {
                    $extra_cost_id = "";
                }
                ...
                $data = array(
                    ...
                    "tour_email_description" => $tour_email_description,
                    "tour_type_id" => $tour_type,
                    "tour_category_id" => $tour_category,
                    "unique_code" => $unique_code,
                    "duration" => $duration,
                    "top_selling_tour" => $top_selling_tour,
                    "upselling_tours" => $upselling_tour,
                    "rating" => $ratings,
                    "feature_image" => $feature_img_name,
                    "banner_image" => $tour_banner_image,
                    "gallery_image" => implode(',', $gallery_img),
                    "extra_services_id" => $extra_service_id,
                    "extra_costs_id" => $extra_cost_id,
                    "is_city_tour" => $is_city_tour,
                    "status" => 1,
                    "video_url" => $video_url,
                    "active_campaign_automation_id" => $active_campaign_automation_id,
                    "meta_title" => $meta_title,
                    "meta_keyword" => $meta_keywords,
                    "meta_description" => $meta_description
                );
                $insert = $this->transfer_tours->insert($data);
                ...
                if ($insert) {
                    ...
                    set_alert('success', _l('_added_successfully', _l('transfer_tours')));
            ...
            $this->set_page_title(_l('transfer_tours') . ' | ' . _l('add'));

            $cdata['tour_types'] = get_tour_types($type = 'transfer');
            $cdata['tour_categories'] = get_tour_categories();
            $cdata['extra_services'] = get_extra_services();
            $cdata['extra_cost'] = get_extra_cost();

            $data['content'] = $this->load->view('admin/transfer_tours/add', $cdata, TRUE);
            $this->load->view('admin/layouts/index', $data);
        }
    }
    ...
    public function istourExists()
    {
        ...
        $check_exist = $this->transfer_tours->get_by($where);

        if ($record_id) {

            $where = array("id" => $record_id);
            $check_exist_byid = $this->transfer_tours->get_by($where);
    ...
    public function update_status()
    {
        ...
        $update = $this->transfer_tours->update_by(array("id" => $tour_id), $data);

        if ($update) {
            if ($this->input->post('is_active') == 1) {
                $response['success'] = true;
                $response['msg'] = 'true';
                $response['alert_msg'] = _l('_activated', _l('transfer_tours'));
            } else {
                $response['success'] = true;
                $response['msg'] = 'false';
                $response['alert_msg'] = _l('_deactivated', _l('transfer_tours'));
            }
        } else {
            $response['success'] = false;
    ...
    public function delete()
    {
        ...
        $gettour = $this->transfer_tours->get_by($where);
        ...
        $deleted = $this->transfer_tours->delete_by(array("id" => $tour_id));
        ...
        if ($deleted) {
            ...
            $response['success'] = true;
            $response['msg'] = _l('_deleted_successfully', _l('transfer_tours'));
        } else {
            $response['success'] = false;
            $response['msg'] = _l('something_wrong');
        }
    ...
    public function edit($id = '')
    {
        ...
        // print_r($this->input->post());
        // exit;
        $this->set_page_title(_l('transfer_tours') . ' | ' . _l('edit'));
        if ($id) {
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if (is_array($gallery_img) && sizeof($gallery_img) > 0) {
                        ...
                        $get_tour = $this->transfer_tours->get_by($where1);
                    ...
                    if ($tour_banner_image) {
                        $data["banner_image"] = $tour_banner_image;
                    }
                    // print_r($data);
                    // exit;
                    ...
                    if ($update) {
                        ...
                        set_alert('success', _l('_updated_successfully', _l('transfer_tours')));
                ...
                $toursData = $this->transfer_tours->get_by($where);

                // echo "<pre>";
                // print_r( $toursData['variation_price']);
                // exit;
                if (is_array($toursData) && sizeof($toursData) > 0) {
                    $toursData['variation_price'] = $this->transfer_tours->getToursPriceDetails(base64_decode($id));
                    $cdata['toursData'] = $toursData;
                    $cdata['tour_types'] = get_tour_types($type = 'transfer');
                    $cdata['tour_categories'] = get_tour_categories();
                    $cdata['extra_services'] = get_extra_services();
                    $cdata['extra_cost'] = get_extra_cost();
                    $data['content'] = $this->load->view('admin/transfer_tours/edit', $cdata, TRUE);
                    $this->load->view('admin/layouts/index', $data);
                } else {
    ...
    public function remove_gallery_img()
    {
        ...
        $where = array("id" => $tour_id);
        $get_tour_data = $this->transfer_tours->get_by($where);
    ...
    public function getCustomPriceLists()
    {
        ...
        // Fetch tours's list
        $memData = $this->transfer_tours->getPriceRows($_POST);
        ...
        foreach ($memData as $tours) {
            ...
            $data[$j]['action'] = json_encode(array('tour_date' => $tours->tour_date, 'tour_availability' => $tours->tour_availability));
            $j++;
            $i++;
        }
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->transfer_tours->countPriceFiltered($_POST),
            "recordsFiltered" => $this->transfer_tours->countPriceFiltered($_POST),
            "aaData" => $data,
        );
