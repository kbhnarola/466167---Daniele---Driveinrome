	...
	public function forgot_password($email)
	{
		...
		if (is_array($user) && sizeof($user)>0)
		{
			...
			if ($this->db->affected_rows() > 0)
			{
				...
				//$message = str_replace('{email_header_logo}', base_url('assets/images/emails/logo.svg'), get_settings('email_header'));
				...
				$data['title'] = $subject;
				$get_new_email_body = str_replace($find, $replace, $template['message']);
				$template_path = 'email_template/';
				$message = $this->load->view($template_path . 'header', $data, TRUE);
        		$message .= $get_new_email_body;
				$message .= $this->load->view($template_path . 'footer', $data, TRUE);

				// $footer_text_img = ["{company_name}", "{ic-1}", "{ic-2}",'{ic-3}'];
				// $footer_text_img_replace   = [get_settings('company_name'),base_url('assets/images/emails/ic-1.png'),base_url('assets/images/emails/ic-2.png') , base_url('assets/images/emails/ic-3.png')];
				
				// $message .= str_replace($footer_text_img, $footer_text_img_replace, get_settings('email_footer'));
				// echo $message;
				// exit;
