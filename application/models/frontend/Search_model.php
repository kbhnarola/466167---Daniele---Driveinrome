...
class Search_model extends CI_Model
{
    ...
    function get_tours_by_name($tour_name = '', $limit = false)
    {
        ...
        if ($limit) {
            return $this->db->select(TBL_TOUR . '.title, ' . TBL_TOUR . '.tour_type_id, ' . TBL_TOUR . '.slug')->from(TBL_TOUR)->where("title LIKE '%$tour_name%' AND status = 1")->order_by(TBL_TOUR . '.id', 'DESC')->limit(5)->get()->result_array();
        } else {
            return $this->db->select(TBL_TOUR . '.title, ' . TBL_TOUR . '.tour_type_id, ' . TBL_TOUR . '.slug')->from(TBL_TOUR)->where("title LIKE '%$tour_name%' AND status = 1")->order_by(TBL_TOUR . '.id', 'DESC')->get()->result_array();
        }
    }
