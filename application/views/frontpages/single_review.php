<section class="section-title review-title">
    <div class="container">
      ...
      if(is_array($tour_title) && sizeof($tour_title)>0) { 
        if($tour_title['title']) { ?>
          <h3><?php echo $tour_title['title'];?></h3>
      <?php } else {?>
          <h3>Review</h3>
      <?php }  } else { ?>
          <h3>Review</h3>
...
<section class="pt-40 pb-80 review-section">
  <div class="container">
        ...
        <div class="single-review-content">
          ...
          <div class="pagination-main">
            ...
            if(is_array($get_previous_review) && sizeof($get_previous_review)>0) { 
              if(get_tour_slug_review($get_previous_review['tour_id'])) { ?>
                <a class="pagination-review prev-btn" href="<?php echo base_url().'reviews/'.get_tour_slug_review($get_previous_review['tour_id']).'/'.$get_previous_review['slug'];?>"><i class="fas fa-chevron-left"></i></a>
              <?php } else { ?>
            ...
            if(is_array($get_next_review) && sizeof($get_next_review)>0) { 
              if(get_tour_slug_review($get_next_review['tour_id'])) { ?>
                <a class="pagination-review next-btn" href="<?php echo base_url().'reviews/'.get_tour_slug_review($get_next_review['tour_id']).'/'.$get_next_review['slug'];?>"><i class="fas fa-chevron-right"></i></a>
            <?php } else { ?>
          ...
          <div class="text-content">
            <h4><?php echo $reviews['title'];?></h4>
             <p><strong><?php if($reviews['review_date']!="" && $reviews['review_date']!="0000-00-00") { echo date("M Y",strtotime($reviews['review_date'])); }?></strong></p>
    ...
    <div class="mt-20 backbtn-wrapper">
      <!-- <a href="<?php //echo base_url().'reviews/'.$this->uri->segment(2);?>" class="btn btn-blue float-left"><i class="fal fa-long-arrow-left"></i>&nbsp;&nbsp;Back to Reviews List</a> -->
      <a href="<?php echo base_url().'tours/'.$this->uri->segment(2);?>" class="btn btn-blue float-left"><i class="fal fa-long-arrow-left"></i>&nbsp;&nbsp;Back to Tours</a>
    </div>
  </div>
</section>
