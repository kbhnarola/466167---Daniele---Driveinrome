...
<?php
  ...
  if (strpos($page_url, 'hindi') > 0){
    $language= 'hi';
  }else if (strpos($page_url, 'chinese') > 0){
    $language= 'cn';  
  }else if (strpos($page_url, 'japanese') > 0){
    $language= 'ja';
  }else if (strpos($page_url, 'russian') > 0){
    $language= 'ru';
  }else if (strpos($page_url, 'thai') > 0){
    $language= 'th';
  }
...
<head>
  ...
  <link rel="canonical" href="<?= current_url(); ?>" hreflang="<?=$language?>"/>
  <meta name="viewport" content="width=device-width, initial-scale=1" language="<?=$language?>">
  <meta name="description" content="<?php echo isset($meta_description) ? $meta_description : '' ?>">
  <meta name="keywords" content="<?php echo isset($meta_keyword) ? $meta_keyword : '' ?>">
  <script src="https://www.google.com/recaptcha/api.js?render=<?= RE_CAPTCHA_V3_SITE_KEY ?>"></script>
  <!-- Google Recaptcha -->
  ...
  <script>
    var CaptchaCallback = function() {
        ...
        if ( $(document).find('#travelLandingRecaptcha').length ) {
          requestTravel_landing = grecaptcha.render('travelLandingRecaptcha', {'sitekey' : '<?php echo get_settings('gr_site_key'); ?>', 'callback' : correctCaptcha_travel_landing});
        }
        
        contactUs = grecaptcha.render('contactUsRecaptcha', {'sitekey' : '<?php echo get_settings('gr_site_key'); ?>', 'callback' : correctCaptcha_contact});
        getacall = grecaptcha.render('getacallFrmRecaptcha', {'sitekey' : '<?php echo get_settings('gr_site_key'); ?>', 'callback' : correctCaptcha_call});
        quickQuote = grecaptcha.render('quickQuoteFrmRecaptcha', {'sitekey' : '<?php echo get_settings('gr_site_key'); ?>', 'callback' : correctCaptcha_quote});
        requestQuote = grecaptcha.render('requestQuoteRecaptcha', {'sitekey' : '<?php echo get_settings('gr_site_key'); ?>', 'callback' : correctCaptcha_quick_quote});        
    };
  ...
  if ($this->uri->segment(1) == "tours" || $this->uri->segment(1) == "availability_ticket" ||  $this->uri->segment(1) == "partners" || $this->uri->segment(1) == "transfer") { ?>
  ...
  <script src="<?= ASSET ?>js/cookie.min.js" async defer></script>
            ...
            <ul class="nav navbar-nav">
              ...
              <li class="dropdown">
                ...
                <ul class="dropdown-menu">
                  ...
                  // get the transfer and shared transfer tour type cities

                  $get_tour_transfer_list_in_menu = get_tour_transfer_city_list_in_menu(array(9, 10));

                  if ($get_tour_transfer_list_in_menu) {

                    foreach ($get_tour_transfer_list_in_menu as $city_list_in_menu) {

                  ?>

                      <li>

                        <a href="<?= base_url('transfers/' . $city_list_in_menu['tour_category_slug']) ?>" <?= ($this->uri->segment(1) == 'transfers' && $this->uri->segment(2) == $city_list_in_menu['tour_category_slug']) ? 'class="active"' : '' ?>><?= $city_list_in_menu['city_title'] ?></a>
              ...
              <li><a href="<?= base_url('umbria-villa') ?>" class="umbriavilla-top-link <?= ($this->uri->segment(1) == 'umbria-villa') ? 'active' : '' ?>">Umbria Villa</a></li>
