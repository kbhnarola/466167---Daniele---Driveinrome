<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Blog_categories_model extends MY_Model
{
	protected $_table = TBL_BLOG_CATEGORIES;
	/**
	 * Constructor for the class
	 */
	public function __construct()
	{
		parent::__construct();

		$this->table=TBL_BLOG_CATEGORIES;
		// Set orderable column fields
        $this->column_order = array(TBL_BLOG_CATEGORIES.'.title',TBL_BLOG_CATEGORIES.'.status');
        // Set searchable column fields
        $this->column_search = array(TBL_BLOG_CATEGORIES.'.title',TBL_BLOG_CATEGORIES.'.status');
        // Set default order
        $this->order = array(TBL_BLOG_CATEGORIES.'.created_at' => 'desc');

	}

    public function getRows($postData){

        $this->_get_datatables_query($postData);
        if($postData['length'] != -1){
            $this->db->limit($postData['length'], $postData['start']);
        }        
        $query = $this->db->get();
        // echo $this->db->last_query();die;
        return $query->result();
    }

    public function countFiltered($postData){
        $this->_get_datatables_query($postData);
        $query = $this->db->get();
        // echo 'Query : '.$this->db->last_query();die;
        return $query->num_rows();
    }

    private function _get_datatables_query($postData){
         
        // $this->db->select(TBL_USERS.'.*');
        $this->db->select(TBL_BLOG_CATEGORIES.'.*');
        $this->db->from($this->table);

        // $this->db->join(TBL_BLOG_CATEGORIES, TBL_BLOG_CATEGORIES.'.id = SUBSTRING_INDEX(SUBSTRING_INDEX('.TBL_BLOGS.'.category_ids, ",", FIND_IN_SET('.TBL_BLOG_CATEGORIES.'.id, '.TBL_BLOGS.'.category_ids)), ",", -1)');

        $i = 0;        
        // loop searchable columns 
        // $this->columns[4][search][value]
        foreach($this->column_search as $item){
            // if datatable send POST for search
            if($postData['search']['value']){
                // first loop
                if($i===0){
                    // open bracket
                    $this->db->group_start();
                    $this->db->like($item, $postData['search']['value']);
                }else{
                    $this->db->or_like($item, $postData['search']['value']);
                }
                
                // last loop
                if(count($this->column_search) - 1 == $i){
                    // close bracket
                    $this->db->group_end();
                }
            }
            $i++;
        }                
         
        if(isset($postData['order'])){
            $this->db->order_by($this->column_order[$postData['order']['0']['column']], $postData['order']['0']['dir']);
        }else if(isset($this->order)){
            $order = $this->order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
        $this->db->group_by(TBL_BLOG_CATEGORIES.'.id');
    }
    public function get_blog_category($where = ''){
        $this->db->select();
        $this->db->from(TBL_BLOG_CATEGORIES);
        if(!empty($where))
            $this->db->where($where);
        else
            $this->db->where(array('status' => 1));

        $query = $this->db->get();
        return $query->result_array();        
    }
    public function get_category_by($where){
        $this->db->select();
        $this->db->from(TBL_BLOG_CATEGORIES);         
        $this->db->where($where);
        $query = $this->db->get();
        return $query->row_array();
    }
    public function insert_blog_category($data){
        if($this->db->insert(TBL_BLOG_CATEGORIES, $data)){
            return $this->db->insert_id();
        }else{
            return FALSE;
        }
    }
    public function delete_category($data){
        if($this->db->delete(TBL_BLOG_CATEGORIES, $data))
            return TRUE;
        else
            return FALSE;
    }
    public function update_category($where, $data){
        $this->db->where($where);
        $this->db->update(TBL_BLOG_CATEGORIES, $data);
        if ($this->db->affected_rows() == '1')
            return true;
        else
            return false;
    }
}
?>