...
class Users_model extends MY_Model
{
    ...
    public function __construct()
    {
        ...
        $this->column_order = array(null, TBL_USERS . '.name', TBL_USERS . '.email', TBL_USERS . '.phone_number', TBL_TAG . '.name', TBL_USERS . '.notes', TBL_USERS . '.subscribe', TBL_USERS . '.created_at', null, 'id');
    ...
    public function getRows($postData)
    {
        ...
        if ($postData['columns'][4]['search']['value'] == 'notag') {
            $this->db->where("(" . TBL_USERS . ".tag_id is NULL OR " . TBL_USERS . " .tag_id = 0)");
        } else if ($postData['columns'][4]['search']['value'] != '') {
            $this->db->where(TBL_TAG . '.name', $postData['columns'][4]['search']['value']);
        }
